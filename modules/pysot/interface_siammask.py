...
import numpy as np
import torch
from modules.pysot.pysot.core.config2 import cfg
from modules.pysot.pysot.models.model_builder import ModelBuilder2
from modules.pysot.pysot.tracker.tracker_builder import build_tracker2
...
class SiamMask():
    def __init__(self, cfg_file, snapshot):
        ...
        # create model
        self.model = ModelBuilder2()
        # load model
        self.model = load_pretrain(self.model, snapshot).cuda().eval()
        # build tracker
        self.tracker = build_tracker2(self.model)
    ...
    def inference(self, img, thres=0.9):
        ...
        x1 = np.min(polygon[0::2])
        x2 = np.max(polygon[0::2])
        y1 = np.min(polygon[1::2])
        y2 = np.max(polygon[1::2])

        box_mask = np.array([x1, y1, x2, y2]).astype(np.float32)
        ...
        return box_mask, box, score, find  # [x1 y1 x2 y2] score
    ...
    def inference_top_k(self, img, top_k=20, keep=5):  # top K boxes
        ...
        box = np.array(box[keeps]).astype(np.float32).reshape(-1, 4)
