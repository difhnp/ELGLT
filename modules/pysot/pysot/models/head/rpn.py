...
from modules.pysot.pysot.core.xcorr import xcorr_fast, xcorr_depthwise
from modules.pysot.pysot.models.init_weight import init_weights
...
class MultiRPN(RPN):
    ...
    def forward(self, z_fs, x_fs):
        ...
        for idx, (z_f, x_f) in enumerate(zip(z_fs, x_fs), start=2):
            ...
            if z_f.size(0) > x_f.size(0):
                c, l = rpn(z_f, x_f.expand(z_f.size(0), x_f.size(1), x_f.size(2), x_f.size(3)))
            else:
                c, l = rpn(z_f.expand(x_f.size(0), z_f.size(1), z_f.size(2), z_f.size(3)), x_f)
        ...
        #     return weighted_avg(cls, cls_weight), weighted_avg(loc, loc_weight)
        # else:
        #     return avg(cls).unsqueeze(0), avg(loc).unsqueeze(0)
