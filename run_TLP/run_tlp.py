...
# ==================================================================================================================== #
    ...
    tracker_name = 'RV1304_OBJ020405_LST03001000_TLP'
    print('===========================================================================================================')
    print('Tracker:', tracker_name)

    # data_name = 'LaSOT'
    # data_dir = '/home/space/Documents/LaSOT/images/'

    data_name = 'TLP'
    data_dir = '/data1/Dataset/TLP/TLP_seqs/'

    vid_list = os.listdir(data_dir)
    vid_list.sort()

    fps_sum = []
    for v_idx, title in enumerate(vid_list):
        box_list = []
        score_list = []
        time_list = []
        fps_list = []
        ...
        """video info"""
        _, bbox_list, frame_num = tools.load_video(data_dir, vid_list, v_idx, 'otb')
        bbox_list = bbox_list[:, 1:5]
        bbox_list = tools.process_ground_truth(bbox_list)

        """get bbox and pos"""
        bbox = bbox_list[0].reshape(4).copy()
        img = tools.load_video_frame(data_dir, vid_list, v_idx, 1, 'otb')
        ...
        for frame_i in range(2, frame_num + 1):
            img = tools.load_video_frame(data_dir, vid_list, v_idx, frame_i, 'otb')


            tic = time()
            predict_box, predict_score = tracker.track(img, np.array(bbox_list[frame_i-1].reshape(4)))
            toc = time()
