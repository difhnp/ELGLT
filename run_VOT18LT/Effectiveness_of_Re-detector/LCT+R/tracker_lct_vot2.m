function [positions, confidence] = tracker_lct_vot(video_path, img_files, pos, target_sz, show_visualization, re_boxes)
%
...
%   sets up the parameters and loads the video information.
    ...
    confidence = zeros(numel(img_files), 1);
    svm_struct=[];
	...
	for frame = 1:numel(img_files),
        ...
        if frame > 1,
            ...
            if max_response<config.motion_thresh,  
                tmp_target_sz = [re_boxes(frame,4), re_boxes(frame,3)];  % H W
                pos = [re_boxes(frame,2), re_boxes(frame,1)] + floor(tmp_target_sz/2);  % cy cx
        ...
        positions(frame,1) = pos(2);
        positions(frame,2) = pos(1);
        positions(frame,3) = target_sz_s(2);
        positions(frame,4) = target_sz_s(1);
        positions(frame,1:2) = positions(frame,1:2) - positions(frame,3:4)/2;
