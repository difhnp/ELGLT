...
def overlap_ratio(r1, r2, mode='xyxy'):
    ...
    if mode == 'xywh':
        # to [x1 y1 x2 y2]
        rec1[:, 2:] = rec1[:, 2:] + rec1[:, :2]
        rec2[:, 2:] = rec2[:, 2:] + rec2[:, :2]
    ...
    zero_idx = np.logical_or(left_line >= right_line, top_line >= bottom_line)
    intersect[zero_idx] = 0
    if ((s_rec1 + s_rec2 - intersect)==0).sum() > 0:
        print('')

    iou = intersect / (s_rec1 + s_rec2 - intersect)
