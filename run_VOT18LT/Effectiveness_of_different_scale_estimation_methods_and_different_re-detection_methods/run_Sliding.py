...
def gen_search_pos(img, last_w, last_h):
    w_z = last_w + 0.5 * (last_w + last_h)
    h_z = last_h + 0.5 * (last_w + last_h)
    s_z = np.sqrt(w_z * h_z)
    s_x = s_z * (255 / 127)
    ...
    win_w = context_xmax - context_xmin
    win_h = context_ymax - context_ymin

    stride_w = win_w // 2
    stride_h = win_h // 2

    num_w = np.ceil(im_sz[1] / stride_w).astype(int)
    num_h = np.ceil(im_sz[0] / stride_h).astype(int)

    arr_x = np.array([[stride_w * i for i in range(num_w)]]).repeat(num_h, axis=0).reshape(-1, 1)
    arr_y = np.array([[stride_h * i for i in range(num_h)]]).repeat(num_w, axis=0).transpose(1, 0).reshape(-1, 1)
    pos = np.concatenate((arr_x, arr_y), axis=1)

    return pos.astype(int)  # [cx cy]
...
class Tracker(object):
    ...
    def track(self, image, gt):
        ...
        if find:
            ...
            pos = gen_search_pos(image, self.first_w, self.first_h)

            skim_box = []
            skim_score = []
            for s_i in range(pos.shape[0]):
                self.tracker.tracker.center_pos = pos[s_i]  # [cx cy]
