...
class Tracker(object):
    ...
    def track(self, image, gt):
        ...
        box_e, score_e, find = self.embedder.inference1(image, box_r, score_r, thres=self.thres_v1)  # [x y x y]
        ...
        if find:
            ...
            if obj_map.max() < self.thres_map:
                ...
                box_r, score_r = self.tracker.inference_top_k(image, top_k=20, keep=5)  # [x y x y]
                box_e, score_e, find = self.embedder.inference1(image, box_r, score_r, thres=self.thres_v2)  # [x y x y]
            if find:
                box = box_e
                score = score_e
            else:
